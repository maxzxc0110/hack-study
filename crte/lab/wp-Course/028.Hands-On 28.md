...
使用Active Directory模块，枚举Foreign Security Principals，注意最后一条结果
...
一旦我们知道有一个ForestTransitive信任，并且SIDFIlteringForestAware是假的，就可以列举production.local上的信任，以确定使用的PAM信任

如果我们试图从bastion.local上的会话访问production.local，我们会发现 会话访问production.local，我们将面临双跳（double hop）问题
